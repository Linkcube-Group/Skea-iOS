...
{
    NSDate *todayDate = [NSDate date];
    NSDate *msgDate = [NSDate dateWithTimeIntervalSince1970:interval/1000];

    if ([todayDate month]>[msgDate month] || [todayDate day]>[msgDate day]+2) {
        return [NSString stringWithFormat:@"%d月%d日 %d:%02d",[msgDate month],[msgDate day],[msgDate Hour],[msgDate minute]];
    }
    else{///在同一个月
        if ([todayDate day]>[msgDate day]+1) {//前天
            return [NSString stringWithFormat:@"前天 %d:%02d",[msgDate Hour],[msgDate minute]];
            
        }
        else if ([todayDate day]>[msgDate day])//昨天
        {
            return [NSString stringWithFormat:@"昨天 %d:%02d",[msgDate Hour],[msgDate minute]];
        }
        else{//今天
            if ([msgDate Hour]<12) {//上午
                return [NSString stringWithFormat:@"上午 %d:%02d",[msgDate hour],[msgDate minute]];
            }
            else{///下午
               return [NSString stringWithFormat:@"下午 %d:%02d",[msgDate hour],[msgDate minute]];
            }
            
        }
    }
    
    return [NSString stringWithFormat:@"%d:%02d",[msgDate hour],[msgDate minute]];
}
@end
